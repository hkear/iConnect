...
use iconnectd::service_manager::{Service, ServiceInstallOptions};
...
impl<'a> CommandHandler<'a> {
        ...
        impl From<PeerRoutePair> for PeerTableItem {
            fn from(p: PeerRoutePair) -> Self {
                ...
                PeerTableItem {
                    ...
                    ipv4: route
                        .ipv4_addr
                        .map(|ip: iconnectd::proto::common::Ipv4Inet| ip.address.unwrap_or_default())
                        .map(|ip| ip.to_string())
                        .unwrap_or_default(),
    ...
    async fn handle_peer_ipv6(&self) -> Result<(), Error> {
        ...
        fn fmt_ipv6_inet(value: Option<iconnectd::proto::common::Ipv6Inet>) -> String {
            value
    ...
    match cli.sub_command {
        ...
        SubCommand::GenAutocomplete { shell } => {
            ...
            if let Some(shell) = shell.to_shell() {
                iconnectd::print_completions(shell, &mut cmd, "easytier-cli");
            } else {
                // Handle Nushell
                iconnectd::print_nushell_completions(&mut cmd, "easytier-cli");
            }
        }
    }
