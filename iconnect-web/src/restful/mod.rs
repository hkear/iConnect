...
use iconnectd::common::config::{ConfigLoader, TomlConfigLoader};
use iconnectd::launcher::NetworkConfig;
use iconnectd::proto::rpc_types;
...
use device::{DeviceRegistry, SharedDeviceRegistry};
...
pub struct RestfulServer {
    ...
    device_registry: SharedDeviceRegistry,
}
...
impl RestfulServer {
    ...
    > {
        ...
        // Add device management routes (does not require login for listing pending devices
        // in practice the admin page handles auth; external callers use internal auth)
        let app = app.merge(device::router().with_state(self.device_registry.clone()));
